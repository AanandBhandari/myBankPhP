<?php
	...
	{
		if(!empty($_POST['first_name'])&&!empty($_POST['last_name'])&&!empty($_POST['email_id'])&&!empty($_POST['password1'])&&!empty($_POST['password2'])&&!empty($_POST['mobile_no'])&&!empty($_POST['acc_no']))
		{
			if(isset($_POST['first_name'])&&isset($_POST['last_name'])&&isset($_POST['email_id'])&&isset($_POST['password1'])&&isset($_POST['password2'])&&isset($_POST['mobile_no']))
			{
				...
				{
					if(strlen($_POST['password1'])>=2&&is_numeric($_POST['mobile_no'])&&strlen($_POST['mobile_no'])==10)
					{
						$first_name = $_POST['first_name'];
						$last_name = $_POST['last_name'];
						$email_id = mysqli_real_escape_string($con,stripslashes($_POST['email_id']));
						$password = md5(mysqli_real_escape_string($con,stripslashes($_POST['password1'])));
						$mobile_no = $_POST['mobile_no'];
						$acc_no = $_POST['acc_no'];
						
						$query = "INSERT INTO $mysqli_db.$mysqli_tb(`first_name`,`last_name`,`email_id`,`password`,`balance`,`mobile_no`,`acc_no`) VALUES('$first_name','$last_name','$email_id','$password','1000','$mobile_no','$acc_no');";
						
						$result = mysqli_query($con,$query);
						if($result)
						{
							$query1 = "SELECT `acc_no` FROM $mysqli_db.`customers` WHERE `email_id`='$email_id'";
							$result1 = mysqli_query($con,$query1);
							
							while($rows_select = mysqli_fetch_assoc($result1))
