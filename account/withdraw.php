	...
	else
	{
		// !empty($_POST['acc_no'])&&
			if(!empty($_POST['amt'])&&!empty($_POST['pass_key']))
			{
				...
				$acc_no = mysqli_real_escape_string($con,stripslashes($_SESSION['acc_id']));
				...
				if(is_numeric($acc_no))
				{
					...
					{
						$query = "SELECT * FROM $mysqli_db.$mysqli_tb WHERE `acc_no`='$acc_no'";
									...
									if($password==$pass)
									{
										if($bal>='100')
										{
											if($amount<=$bal)
											{
												...
												if($own_balance>='100')
												{
														$query_withdraw = "UPDATE $mysqli_db.$mysqli_tb SET `balance`='$own_balance' WHERE `acc_no`='$acc_no'";
														...
														//	echo "<center> <p style='padding: 5px;background-color:red;'> Successfully Withdrawed from Account.</p>	</center>";
																	...
																	$query_5 = "INSERT INTO $mysqli_db.$mysqli_transaction_withdraw(`acc_no`,`wid`) VALUES('$acc_no','$transfer_text');";
																		
																	if(mysqli_query($con,$query_5))
																	{
				...
				echo "<center> <p style='padding: 5px;background-color:red;'> Your account number or passkey is empty. Please Try Again. </p>	</center>";
			}
