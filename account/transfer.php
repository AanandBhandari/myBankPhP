<?php 
	...
	else
	{
			...
			if(!empty($_POST['acc_no'])&&!empty($_POST['amt'])&&!empty($_POST['pass_key']))
			{
				...
				$acc_no = mysqli_real_escape_string($con,stripslashes($_POST['acc_no']));
				$password = md5(mysqli_real_escape_string($con,stripslashes($_POST['pass_key'])));
				$amount = mysqli_real_escape_string($con,stripslashes($_POST['amt']));

				if(is_numeric($acc_no))
				{
					if(is_numeric($amount))
					{
						$query = "SELECT * FROM $mysqli_db.$mysqli_tb WHERE `acc_no`='$acc_id'";
						$result = mysqli_query($con,$query);
						if(!$result)
							echo "<center> <p style='padding: 5px;background-color:red;'> Something Went Wrong. Please Try Again. </p>	</center>";
						else
						{
								$query_1 = "SELECT * FROM $mysqli_db.$mysqli_tb WHERE `acc_no`='$acc_no'";
										...
										while($row = mysqli_fetch_assoc($result))
										{
											...
											if($password==$pass)
											{
												if($amount<=$bal)
												{
													...
													if($own_balance>='100')
													{
															$query_transfer = "UPDATE $mysqli_db.$mysqli_tb SET `balance`='$own_balance' WHERE `acc_no`='$acc_id'";
															$result_transfer = mysqli_query($con,$query_transfer);

															$query_2 = "SELECT * FROM $mysqli_db.$mysqli_tb WHERE `acc_no`='$acc_no'";
															...
															if($count_2==1)
															{
																...
																$query_transfer2 = "UPDATE $mysqli_db.$mysqli_tb SET `balance`='$other_balance' WHERE `acc_no`='$acc_no'";
																$result_transfer2 = mysqli_query($con,$query_transfer2);
																if(!$result_transfer2)	{
																	echo "<center> <p style='padding: 5px;background-color:red;'> Something went wrong.Please try again. </p>	</center>";
																}									
																...
																else
																{
																	...
																	$query_3 = "INSERT INTO $mysqli_db.$mysqli_transaction_transfer(`acc_no`,`tid`) VALUES('$acc_no','$transfer_text');";
																		
																	/*Other's Account*/
																	$transfer_text_2 = "Transfered Rs.".$amount." into Account Number - ".$acc_no.". Transaction ID - ".$token_save;
																	$query_4 = "INSERT INTO $mysqli_db.$mysqli_transaction_transfer(`acc_no`,`tid`) VALUES('$acc_id','$transfer_text_2');";

																	if(mysqli_query($con,$query_4) && mysqli_query($con,$query_3))
																	{
