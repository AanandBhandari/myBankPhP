<?php 
	...
	{	
		...
		$query_withdraw = "SELECT * FROM $mysqli_db.$mysqli_transaction_withdraw WHERE `acc_no`='$acc_number' ORDER BY `dow` DESC LIMIT 5";
		$result1 = mysqli_query($con,$query_withdraw);

		$query_transfer = "SELECT * FROM $mysqli_db.$mysqli_transaction_transfer WHERE `acc_no`='$acc_number' ORDER BY `dot` DESC LIMIT 5";
		$result2 = mysqli_query($con,$query_transfer);
